...
function clipAddDates(startDate, endDate, addDates) {
  const result = [];
  for (const date of addDates.valueSeq().toArray()) {
...
function hasRecurrence(startDate, endDate, days, addDates, cronDates, date) {
  ...
  const isCron = cronDates.valueSeq().toArray().indexOf(date) !== -1;
  const isInsideAdd = addDates.valueSeq().toArray().indexOf(date) !== -1;
...
function getDatesSummary(addDates, cronDates, type) {
  addDates = new Shredder(addDates).valueSeq().toArray();
  cronDates = new Shredder(cronDates).valueSeq().toArray();
  const array = [];
